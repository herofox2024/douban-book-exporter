...
#### 方法一：使用手动输入的 Douban ID（推荐）
1. 直接点击Chrome工具栏中的扩展图标，打开扩展弹窗
2. 在「豆瓣ID输入」框中输入豆瓣用户名或ID（例如：herofox2024）
3. 点击「开始爬取」按钮，等待爬取完成
4. 选择导出格式（CSV或HTML），点击「导出」按钮
5. 选择保存位置，完成导出
...
2. **输入豆瓣ID**
   - 在「豆瓣ID输入」框中输入豆瓣用户名或ID（例如：herofox2024）
   - 无需登录豆瓣网页，直接输入ID即可使用

3. **爬取数据**
   - 点击「开始爬取」按钮
   - 等待爬取进度达到100%
   - 扩展会在后台自动爬取数据，不影响当前页面浏览
...
1. **登录豆瓣账号**
   - 打开豆瓣网站（`https://www.douban.com`）并登录
